...
def check_availability():
    ...
    try:
        ...
        for future_date in future_dates_to_check:
            ...
            try:
                # オプションが埋まるまで待機
                time_dropdown_options = WebDriverWait(driver, 20).until(
                    lambda d: d.find_elements(By.XPATH, "//select[@id='customSelectedTimeSlot']/option") and len(d.find_elements(By.XPATH, "//select[@id='customSelectedTimeSlot']/option")) > 1
                )
                ...
                # デバッグ用ログ（必要に応じて有効化）
                # for i, o in enumerate(time_options):
                #     t = o.text
                #     v = o.get_attribute("value")
                #     print(f"[opt{i}] text={repr(t)} value={v}")
                #     print("   cps:", " ".join(hex(ord(c)) for c in t))
